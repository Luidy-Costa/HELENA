...
import { Activity, Clock, AlertTriangle, Search, Plus, FileText, Calendar, Hash, ArrowLeft } from 'lucide-react';
import DashboardLayout from '../layouts/DashboardLayout';

export default function HospitalInterna() {
  const navigate = useNavigate();

  // Dados falsos (Mock) dos pacientes baseados no seu design
  const pacientes = [
    {
      id: 'PRN-2026-001',
  ...
  return (
    <DashboardLayout>
      ...
      {/* Botão de Voltar para o Painel Geral */}
      <button 
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
        ...
        {/* Barra de Pesquisa e Botão Nova Predição */}
        <div className="flex flex-col md:flex-row gap-4 mb-8">
          ...
          <button 
            onClick={() => navigate('/formulario-predicao')} // Rota futura para o formulário
        ...
        <div className="space-y-4">
          {pacientes.map((paciente, index) => (
            <div key={index} className="flex flex-col md:flex-row items-center justify-between p-5 border border-gray-100 rounded-2xl shadow-sm hover:shadow-md transition-shadow bg-white">
              ...
              {/* Botão Direita */}
              <button 
                onClick={() => navigate('/perfil-paciente')} // Rota futura para perfil do paciente
